...
class Model_QuarterHour {
    ...
    public function getSunlightData() {
        $query = DB::query(Database::SELECT, "SELECT datetime, sunlight FROM ( SELECT * FROM quarter_hour ORDER BY id_quarter_hour DESC LIMIT 20 ) sub ORDER BY id_quarter_hour ASC");
